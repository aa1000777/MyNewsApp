...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	// Fields
	// ===========================================================
	private boolean nextflag = true;//用于控制上一页下一页的显示
	private boolean newsendflag=true;//是否为第一页和最后一页（回弹效果的控制）
...
//					setRefreshingInternal(true);
	...
	protected void resetHeader() {
		System.out.println("resetHeader");
...
//			footerLayout.reset();
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_news_mode)) {
			mode = a.getInteger(R.styleable.PullToRefresh_news_mode,
	...
	private boolean pullEvent() {
		...
		if (newHeight != 0) {
			if(newHeight < 0 && nextflag){
				this.newspage = NewsGlobal.pullLabelDOWN;
				for (int i = 0; i < NewsGlobal.newsNO.length; i++) {
					if (NewsGlobal.NEWSNO.equals(NewsGlobal.newsNO[i])) {
						if (i > 0) {
							this.newsendflag=true;
							break;
						} else {
							this.newsendflag=false;
							this.newspage = NewsGlobal.pullLabelDOWNend;
						}
					}
				}
				headerLayout.pullToRefresh(this.newspage);
			}
			if (newHeight > 0 && nextflag) {
				this.newspage = NewsGlobal.pullLabelUP;
				for (int i = 0; i < NewsGlobal.newsNO.length; i++) {
					if (NewsGlobal.NEWSNO.equals(NewsGlobal.newsNO[i])) {
						if (i < NewsGlobal.newsNO.length - 1) {
							...
							this.newsendflag=false;
							this.newspage = NewsGlobal.pullLabelUPend;
						}
					}
				}
				footerLayout.pullToRefresh(this.newspage);
			} 

			if (state == PULL_TO_REFRESH && headerHeight < Math.abs(newHeight)) {
				state = RELEASE_TO_REFRESH;
				// this.news = GetDataFromSQL.getNewsByNewsNo(context,
				// Global.NEWSNO);
				nextflag=false;
				switch (currentMode) {
				case MODE_PULL_UP_TO_REFRESH:
					this.newspage = NewsGlobal.releaseLabel;
					// if (news!=null) {
					...
					footerLayout.releaseToRefresh(this.newspage);
					break;
				case MODE_PULL_DOWN_TO_REFRESH:
					this.newspage = NewsGlobal.releaseLabel;
					// 下拉
					...
					headerLayout.releaseToRefresh(this.newspage);
					break;
				}
				return true;
